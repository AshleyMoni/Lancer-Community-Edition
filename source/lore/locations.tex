...
\textbf{Cradle: \textit{ANDES} Line} -- Aconcagua Station, Cerro Bonete Station, Galan Station, etc

\textbf{Ring 1: \textit{ROCKY MOUNTAIN} Line} -- Elbert Station, Lincoln Station, Castle Station, etc

\textbf{Ring 2: \textit{KUNLUN} Line} -- Kongur Tagh Station, Karakoram Station, Mayakovsky Station, etc

\textbf{Ring 3: \textit{URAL} Line} -- Manaraga Station, Elbrus Station, Iremel Station, etc

\textbf{Ring 4: \textit{ATLAS} Line} -- Toubkal Station, Ouanoukrim Station, M’Goun Station, etc

\textbf{Ring 5: \textit{HIMALAYA} Line} -- Everest Station, Kanchenjunga Station, Annapurna Station, etc

\textbf{Ring 6: \textit{ALTAI} Line} -- Belukha Station, Nairamdal Station, Kharkhiraa Station, etc

\textbf{Ring 7: \textit{CARPATHIAN} Line} -- Gerlachovsky Station, L’adovy Station, Moldoveanu Station, etc

\textbf{Ring 8: \textit{SIERRA MADRE} Line} -- Mohinora Station, Peak Station, Bridger Station, etc

\textbf{Ring 9: \textit{VINDHYA} Line} -- Kalumar Station, Dhupgarh Station, Mahendragiri Station, etc

\textbf{Ring 10: \textit{CASCADE} Line} -- Rainier Station, Adams Station, Hood Station, etc

\textbf{Ring 11: \textit{ANNAMITE} Line} -- Phou Bia Station, Phu Xai Lai Leng Station, Ngoc Linh Station, etc.

The Annamite Line is the current ``edge'' of Union space. Beyond that line is uncharted territory.

\subsection{Non Human Life}
Non-human life is common in \textit{Lancer}; non-human sentient life (i.e. alien civilization) is unique in
how rare it is.

Contents of the module \textit{No Room For A Wallflower} dives into this topic more, but generally
...
\textit{Colonial Settlement} - A colonial settlement can range from the first generation to populate a
...
Artificial gravity \textit{can} be generated, but in the narrative present it is unstable and is the result of
...
Interstellar travel is common for certain classes of people in \textit{Lancer}: military personnel,
...
Time, in \textit{Lancer}, is a rare thing: a commodity that can be exhausted. Whether you’re on a core
...
blink with the aid of a blink gate, may best be measured in how much \textit{time} it will take to reach
...
For interstellar space travel, \textit{velocity} causes time dilation. Time, to the person on the ship,
...
difference in time grows, stretches, \textit{dilates}. It is possible, depending on speed, that for
...
could cost a traveller three years subjective time -- time as \textit{they} experience it -- and \textit{thirty}
...
The inhabited galaxy in \textit{Lancer} is complex and, seemingly, contradictory: it is both a polyglot mix
...
Examples of a common society in \textit{Lancer} are listed below.
